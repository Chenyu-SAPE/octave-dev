...
public class HelicopterBlow : MonoBehaviour {
  ...
  public void Start () {
    ...
    resultDisplay = GameObject.Find("Text2");
    blowDisplay = GameObject.Find("Text1");
  ...
  private void DeriveBlow() {
    ...
    if (pitchValue>0) {
        ...
        //D note (re) is 440
        //E note (mi) is 165
        //F note (fa) is 500-520
        //G note (sol) is 190 (though sometimes shows up as 395 for some reason)
        //A note (la) is 220ish
        //B note (ti) is 230-240
        //C note (doh) is 500
       
        //float pitchLevel = 1f;
        targetPosition = new Vector3(playerSpaceship.transform.position.x, pitchValue*0.1f, playerSpaceship.transform.position.z + 1);
      
        //move spaceship
        playerSpaceship.transform.position = Vector3.Lerp(playerSpaceship.transform.position, targetPosition,  1 * Time.deltaTime);

    } else {

        //playerSpaceship.transform.position = Vector3.Lerp(playerSpaceship.transform.position, new Vector3(0, 1, playerSpaceship.transform.position.z), 1 * Time.deltaTime);
